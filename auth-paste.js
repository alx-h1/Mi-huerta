(()=>{
  async function init(){
    ...
    if(!authForm||!authView)return;

    if(!document.getElementById('magicLinkPasteBox')){
      const box=document.createElement('div');
      box.id='magicLinkPasteBox';
      box.className='stack';
      box.style.marginTop='16px';
      box.innerHTML=`
        <div style="text-align:center;opacity:.7">— activar este ícono una sola vez —</div>
        <p style="margin:0">Si ya generaste un <strong>enlace de activación</strong> desde Safari, pégalo aquí. No necesitas copiar el enlace del correo.</p>
        <label>Enlace de activación
          <input id="magicLinkPasteInput" type="url" inputmode="url" autocomplete="off" placeholder="Pega aquí el enlace de activación" />
        </label>
      ...
      document.getElementById('magicLinkPasteBtn').addEventListener('click',async()=>{
        ...
        if(!raw)return setMsg('Pega primero el enlace de activación.',true);
        let u;
        ...
        setMsg('Listo. Este dispositivo quedó activado.');
      });
    }

    const standalone=window.matchMedia?.('(display-mode: standalone)')?.matches || window.navigator.standalone===true;
    const setupDialog=document.getElementById('setupDialog');
    if(!standalone&&setupDialog){
      const {data}=await db.auth.getSession();
      if(data.session&&!setupDialog.open)setTimeout(()=>setupDialog.showModal(),250);
    }
  }
