...
function dailyTasksFor(c){
  ...
  if(age>30){const feedAgo=typeof lastLogDays==='function'?lastLogDays(c.id,'fertilizacion'):null;if(feedAgo===null||feedAgo>=g.feed)add('nutricion','🌿','Evaluar nutrición','Observa color y vigor. Fertiliza únicamente si corresponde y evita aplicar por rutina sin revisar la planta.','normal')}
...
function renderDailyChecklist(){
  const box=document.getElementById('todayList'),progress=document.getElementById('todayProgress');if(!box||typeof crops==='undefined')return;
  const items=makeDailyChecklist(),done=dailyDone(),completed=items.filter(x=>done[x.id]).length;if(progress)progress.textContent=`${completed} / ${items.length}`;
...
function buildDailyEmail(){
  const items=makeDailyChecklist(),done=dailyDone(),completed=items.filter(x=>done[x.id]),pending=items.filter(x=>!done[x.id]);
  const todayLogs=(typeof logs!=='undefined'?logs:[]).filter(l=>String(l.occurred_at||'').slice(0,10)===today());
  const lines=[`Resumen diario de Mi Huerta — ${fmt(today())}`,'',`Checklist: ${completed.length} de ${items.length} completadas`,''];
  ...
  if(todayLogs.length){lines.push('Actividades registradas hoy:');todayLogs.forEach(l=>{const c=(typeof crops!=='undefined'?crops:[]).find(x=>x.id===l.crop_id);lines.push(`• ${c?.name||'Cultivo'}: ${l.log_type}${l.notes?' — '+l.notes:''}`)});lines.push('')}
