...
$('authForm').addEventListener('submit',async e=>{e.preventDefault();const email=$('email').value.trim();msg('Enviando enlace…');const {error}=await db.auth.signInWithOtp({email,options:{emailRedirectTo:window.location.origin+window.location.pathname}});msg(error?error.message:'Revisa tu correo y abre el enlace para entrar. En este dispositivo la sesión quedará guardada.',!!error)});
