...
$('signupBtn').addEventListener('click',async()=>{msg('Creando cuenta…');const {error}=await db.auth.signUp({email:$('email').value.trim(),password:$('password').value});msg(error?error.message:'Cuenta creada. Si Supabase pide confirmación, revisa tu correo. Luego el dueño de la huerta podrá agregarte.',!!error)});
...
$('inviteForm').addEventListener('submit',async e=>{e.preventDefault();const email=$('inviteEmail').value.trim();const {data,error}=await db.rpc('add_garden_member_by_email',{p_garden_id:garden.id,p_email:email});if(error)return alert(error.message);if(data==='NO_USER')return alert('Ese correo todavía no tiene cuenta en Mi Huerta. Pídele que cree la cuenta primero y vuelve a intentarlo.');$('inviteForm').reset();$('inviteDialog').close();alert('Listo. Esa persona ya puede entrar y editar esta misma huerta.')});
