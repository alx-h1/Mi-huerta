...
$('authForm').addEventListener('submit',async e=>{e.preventDefault();const email=$('email').value.trim();const password=$('password').value;msg('Entrando…');const {error}=await db.auth.signInWithPassword({email,password});msg(error?(error.message==='Invalid login credentials'?'Correo o contraseña incorrectos.':error.message):'',!!error)});
